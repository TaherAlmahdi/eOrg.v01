// app/lib/books.ts
export * from './books/book-details';
export * from './books/extractors';
export * from './books/hierarchy';
export * from './books/utils';
// ১. টাইপগুলো রিকানেক্ট করা
export * from './books/types';

// ২. ফেচার ও মূল ফাংশনগুলো রিকানেক্ট করা
export * from './books/fetchers';
export { getBookDirectoryBySlug as getBookDirectoryBySlugFromToc } from './books/toc';

// ৩. সর্টিং ও হেল্পার ফাংশনগুলো রিকানেক্ট করা
// export * from './books/sorters';
// export * from './books/helpers';
...
function slugify(text: string): string {
  ...
  return String(text)
    ...
    .toLowerCase()
    .replace(/\s+/g, '-')
    .replace(/[^\w\u0980-\u09FF\-]/g, ''); // বাংলা বর্ণমালা ও হাইফেন সাপোর্ট / এখানে পরিবর্তন আছে।
}
...
export function generateGenreLinks(
  ...
  return genresList.map((gName) => ({
    name: gName,
    link: `/genre/${getSlug('genres', gName)}`, //এখানে পরিবর্তন আছে।
  }));
}
