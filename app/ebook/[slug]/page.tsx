...
export default async function ChildNovelPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const novel = await getNovelData(slug);

  if (!novel) notFound();

  // সাইডবার সোর্স নির্ধারণ: যদি প্যারেন্ট থাকে তবে প্যারেন্ট থেকে ডাটা নেবে, নাহলে নিজে থেকেই।
  const sidebarSource = novel.parent ? novel.parent.node : novel;
    ...
    <article className="min-h-screen bg-white pb-20 max-w-[1440px] mx-auto font-sans">
      ...
      <div className="border-none bg-gray-50 py-3 px-4 lg:px-8 rounded-lg mb-2">
        <nav className="flex flex-wrap items-center gap-y-2 gap-x-2 text-sm text-gray-500 font-medium">
          ...
          {novel.parent && (
            <>
              <Link href={`/ebook/${sidebarSource.slug}`} className="hover:text-[#008080]">
                {sidebarSource.title}
              </Link>
              <span className="text-gray-300">/</span>
            </>
      ...
      {/* Title Header */}
      <div className="bg-[#669999] p-5 border border-[#669999]">
