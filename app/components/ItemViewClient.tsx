...
/**
 * বাংলা টেক্সট সার্চের জন্য Normalize
 */
...
/**
 * বিভিন্ন ধরনের field থেকে text বের করা
 */
...
/**
 * ============================================================
 * বাংলা আদ্যক্ষর নির্ধারণ (সংশোধিত)
 * ============================================================
 */
...
export default function ItemViewClient({
  ...
  /**
   * স্ক্রিন সাইজ অনুযায়ী pagination size
   */
  useEffect(() => {
  ...
  const currentPrakaronSlug = itemSlug
    ? slugify(itemSlug)
  ...
  const sortedItems = useMemo(() => {
    ...
    return [...initialItems].sort((a: any, b: any) => {
      ...
      const titleA = extractFieldText(a.title, "")
        .normalize('NFC');

      const titleB = extractFieldText(b.title, "")
        .normalize('NFC');
  ...
  const availableLetters = useMemo(() => {
    ...
    sortedItems.forEach((entry: any) => {
      ...
      if (
        ...
        const initial = getBengaliInitial(title);
  ...
  /**
   * ============================================================
   * ৩. Search + আদ্যক্ষর filtering
   * ============================================================
   */
  const filtered = useMemo(() => {

    const normalizedQuery =
      normalizeBengaliText(search);


    return sortedItems.filter((entry: any) => {

      const rawTitle = extractFieldText(
        entry.title,
        "শিরোনামহীন"
      );

      const rawBook = extractFieldText(
        entry.bookTitle || entry.book,
        ""
      );

      const rawAuthor = extractFieldText(
        entry.author,
        ""
      );
       ...
       */
      return (
        normalizeBengaliText(rawTitle)
          .includes(normalizedQuery) ||

        normalizeBengaliText(rawBook)
          .includes(normalizedQuery) ||

        normalizeBengaliText(rawAuthor)
          .includes(normalizedQuery)
      );
  ...
  /**
   * Search অথবা letter পরিবর্তন হলে
   * pagination প্রথম থেকে শুরু
   */
  useEffect(() => {
    ...
    <main className="w-full font-tarunima px-0 py-0 space-y-3">
      ...
      <nav className="w-full bg-[#7575a3] border-b border-gray-200 py-2 px-3 text-white overflow-x-auto no-scrollbar shadow-xs rounded-t">
        ...
        <div className="max-w-full mx-auto text-sm flex items-center whitespace-nowrap">
          ...
          <Link
            href="/"
            className="shrink-0 hover:text-teal-200 transition-colors"
          >
            <Home size={16} />
          </Link>

          <span className="mx-2 text-white/50 shrink-0">
            /
          </span>

          <Link
            href="/items"
            className="hover:text-teal-200 transition-colors shrink-0"
          >
            প্রকরণ
          </Link>

          {displayTitle &&
            displayTitle !== 'সকল আইটেম' && (
              <>
                <span className="mx-2 text-white/50 shrink-0">
                  /
                </span>

                <span className="text-teal-100 shrink-0">
                  {displayTitle}
                </span>
              </>
            )}
      ...
      <div className="mb-3 space-y-3 bg-white/95 backdrop-blur-md shadow-xs">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            ...
            {filtered.length > 0 && (
              ...
              <p className="text-xs md:text-sm text-gray-500">
                ...
                <span className="w-full text-xs md:text-sm font-normal text-teal-700 bg-teal-100/70 px-2 py-1 border-t border-teal-200 inline-block">
                  ...
                  সর্বমোট{" "}
                  {toBengaliNumber(filtered.length)}
                  টি{" "}
                  {displayTitle || 'নির্বাচিত প্রকরণ'}
                  {" "}রয়েছে
          ...
          <div className="relative w-full md:w-80 shrink-0">
            ...
            <Search
              className="absolute w-4 h-4 text-teal-600 -translate-y-1/2 left-3 top-1/2 pointer-events-none"
            />
              ...
              (item: any, idx: number) => {
                ...
                const resolvedTitleSlug =
                  item.slug
                    ...
                    : (
                        item.itemSlug
                          ? String(item.itemSlug).trim()
                          : itemTitle
                      );


                const pageUrl =
                  `/item/${currentPrakaronSlug}/${encodeURIComponent(
                    resolvedTitleSlug
                  )}`;
